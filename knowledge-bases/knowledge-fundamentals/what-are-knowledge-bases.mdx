...
graph TB
    %% User Input Phase
    User[👤 User] --> |"1. Create Data Resource"| LoadAPI["/api/data-resources<br/>POST"]
...
graph TB
    %% User Input
    User[👤 User] --> |"1. Add Knowledge Base"| AddKB["Add Knowledge Base"]
    User --> |"2. Train Knowledge Base"| TrainKB["Train Knowledge Base"]
    
    %% Data Loading Process
    AddKB --> |"Load Data"| DataSources["Data Sources"]
    DataSources --> |"Websites"| WebData["🌐 Web Content"]
    DataSources --> |"Documents"| DocData["📄 PDFs & Files"]
    DataSources --> |"APIs"| APIData["🔌 Confluence, Notion, etc."]
    DataSources --> |"Databases"| DBData["🗄️ CSV, SQL Data"]
    
    %% Content Processing
    WebData --> |"Extract Text"| ContentProcessor["📝 Content Processor"]
    DocData --> ContentProcessor
    APIData --> ContentProcessor
    DBData --> ContentProcessor
    
    ContentProcessor --> |"Save Raw Data"| DataStorage["💾 Data Storage"]
    
    %% Training Process
    TrainKB --> |"Read Data"| DataReader["📖 Read Stored Data"]
    DataReader --> |"Split into Chunks"| TextChunker["✂️ Text Chunking<br/>(Break into pieces)"]
    TextChunker --> |"Create Embeddings"| EmbeddingEngine["🧠 AI Embedding Engine<br/>(Convert text to numbers)"]
    
    %% Vector Storage
    EmbeddingEngine --> |"Store Vectors"| VectorDB1["🗃️ Vector Database 1<br/>(Qdrant)"]
    EmbeddingEngine --> |"Store Vectors"| VectorDB2["🗃️ Vector Database 2<br/>(Weaviate)"]
    
    %% Search Process
    User --> |"Ask Questions"| SearchEngine["🔍 Search Engine"]
    SearchEngine --> |"Find Similar Content"| VectorSearch["🔎 Vector Search"]
    VectorSearch --> |"Get from DB1"| VectorDB1
    VectorSearch --> |"Get from DB2"| VectorDB2
    
    VectorDB1 --> |"Rank Results"| ResultRanker["📊 Result Ranking<br/>(Sort by relevance)"]
    VectorDB2 --> ResultRanker
    ResultRanker --> |"Best Answers"| FinalAnswer["✅ Final Answer"]
    
    %% Status Updates
    AddKB --> |"Progress Updates"| StatusTracker["📈 Status Tracker"]
