...
AI Agents in Tars use advanced language models (LLMs) to intelligently manage conversations. The LLM dynamically decides whether to retrieve knowledge, execute tools, or generate direct responses based on user input:
...
### How does a Question Answering Agent work in Tars?

A Question Answering Agent is a specialized agent optimized for information retrieval. It uses RAG (Retrieval-Augmented Generation) technology to search your knowledge base and provide accurate, context-aware answers:

```mermaid
flowchart TD
    A(User Input) --> B(Question Answering Agent)
...
<Frame>
<img src="/images/dashboard/Live Chat.svg"/>
</Frame>
...
<Step title="Live Chat Gambit triggered">
